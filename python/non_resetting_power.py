...
def plot_customer_daily_energy():
    ...
    vpl = sa.Table('view_power', metadata, autoload=True)
    ...
    for c in circuit_list:
        ...
        ax[0].plot_date(dates, power, mfc='#dddddd')
        ax[0].set_xlabel('Date')
        ax[0].set_ylabel('Daily Watthours')
        ax[0].set_xlim((date_start, date_end))
        ax[0].set_title(filename)
        ax[1].plot_date(dates, power, mfc='#dddddd', linestyle='-', color='k')
